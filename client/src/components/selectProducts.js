...
//--- removing { withStyles } from line 4 b/c it is not being used
...
const SelectProd = () => {
  ...
  for (const [index, value] of prodname.entries()) {
    proditems.push(
      <FormControlLabel
        control={
          <Checkbox
            /*checked={state.checkedA} onChange={handleChange2}*/ key={index}
            name={value}
          />
        }
        label={value}
  ...
  //---pseudo coding out below const b/c not in use
  //   const pushedhere = [];

  //--- pseudo coding our below function b/c not in use
  //   function pushhh() {
    ...
    <div>
      ...
      <Grid container spacing={3}>
        ...
        {/* oranges {chroma.scale(['#c35817','#e55b3c'])
                    .mode('lch').colors(15)} */}
        {chroma.scale(["#edda74", "#c68e17"]).mode("lch").colors(15)}
